    ...
    public async Task<List<Product>> ScrapeProductsAsync()
    {
        ...
        var productNodes =
            doc.DocumentNode.SelectNodes(
                "//section[contains(@class, 'coles-targeting-ProductTileProductTileWrapper')]");
        ...
        foreach (var productNode in productNodes)
        {
            ...
            var price = decimal.Parse(productNode.SelectSingleNode(".//span[@class='price-dollars']").InnerText
                .Replace("$", ""));
            var ratingNode = productNode.SelectSingleNode(".//div[@class='stars']");
            var rating = ratingNode != null
                ? int.Parse(ratingNode.GetAttributeValue("aria-label", "").Replace("stars", ""))
                : 0;
